...
def ml_objective(trial, trial_inputs, trial_labels):
    ...
    param_grid = {
        "n_estimators": trial.suggest_int("n_estimators", 10, 1000, step=1),
        "max_leaves": trial.suggest_int("max_leaves", 2, 70, step=1),
        "grow_policy": trial.suggest_categorical("grow_policy", ["depthwise", "lossguide"]),
        "learning_rate": trial.suggest_float("learning_rate", 1e-3, 1, log=True),
        "gamma": trial.suggest_float("gamma", 1e-8, 10, log=True),
        "min_child_weight": trial.suggest_float("min_child_weight", 1e-2, 50, log=True),
        ...
        "colsample_bylevel": trial.suggest_float("colsample_bylevel", 0.4, 1, step=0.05),
        "colsample_bynode": trial.suggest_float("colsample_bynode", 0.4, 1, step=0.05),
    ...
    for idx, (train_idx, test_idx) in enumerate(cv.split(trial_inputs, trial_labels)):
        ...
        model = XGBRegressor(**param_grid, objective="reg:squarederror", max_depth=3, n_jobs=-1, random_state=42)
