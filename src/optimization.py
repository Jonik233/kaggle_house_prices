...
def ml_objective(trial, trial_inputs, trial_labels):
    ...
    param_grid = {
        ...
        "max_features": trial.suggest_categorical("max_features", [0.3, 0.5, 0.6, 0.8, "sqrt", "log2", None]),
        "max_leaf_nodes": trial.suggest_int("max_leaf_nodes", 2, 60, step=1),
        "max_samples": trial.suggest_float("max_samples", 0.1, 1, step=0.05)
    }
    ...
    for idx, (train_idx, test_idx) in enumerate(cv.split(trial_inputs, trial_labels)):
        ...
        model = RandomForestRegressor(**param_grid, max_depth=8, n_jobs=-1, random_state=42)
